// Cleaner job hulls + proportional status-mix shading, with robust geometry ops.
// Strategy:
//   1) Group by job.
//   2) DBSCAN split (wide separation).
//   3) Long-edge pruning to avoid “bridges” inside a DBSCAN group.
//   4) Concave hull per sub-cluster, smoothed via buffer out/in.
//   5) Carve overlaps **pairwise** (no turf.union) with safe difference.
//   6) Stroke = dominant status color; Fill = linear gradient by status mix.
//   7) Job name label (non-interactive) stays as before.
...
/* Second pass: prune long edges (avoid single long “bridges”) */
function splitByLongEdges(poles, scaleM) {
  if (poles.length <= 3) return [poles.slice()];
  const CUT = 2.4 * scaleM;
...
/* Build a smooth hull from lng/lat pairs */
...
/* ---------- robust geometry ops (NO turf.union) ---------- */
function makeValid(g) {
  try {
    const cleaned = turf.cleanCoords(g);
    // buffer(0) often fixes ring orientation/self-intersections
    return turf.buffer(cleaned, 0, { units: 'kilometers' }) || cleaned;
  } catch { return g; }
}

/** Safe difference: returns A\B or null if fully eaten/invalid */
function safeDiff(A, B) {
  try {
    // quick reject
    if (!turf.booleanIntersects(A, B)) return A;
    const d = turf.difference(A, B);
    if (d) return turf.cleanCoords(d);
    // retry with make-valid
    const d2 = turf.difference(makeValid(A), makeValid(B));
    if (d2) return turf.cleanCoords(d2);
  } catch {
    try {
      const d3 = turf.difference(makeValid(A), B);
      if (d3) return turf.cleanCoords(d3);
    } catch {}
  }
  return null;
}
...
export function buildJobAreas(map, poles, byKey) {
  ...
  // carve overlaps pairwise (largest first), NO union
  const carved = [];
  const sorted = raw.slice().sort((a,b)=> turf.area(b.feature) - turf.area(a.feature));
  for (const item of sorted) {
    let geom = item.feature;
    // subtract every prior carved geom if it intersects
    for (const prev of carved) {
      const bbA = turf.bbox(geom), bbB = turf.bbox(prev.feature);
      if (!bboxIntersects(bbA, bbB)) continue;
      const diff = safeDiff(geom, prev.feature);
  ...
  for (const { job, feature, poles: plist } of carved) {
    ...
    for (const p of plist) {
      ...
      if (rel && rel.length) {
        ...
        for (const pred of order) { const hit = ss.find(pred); if (hit) {
          s = hit.startsWith('Not Approved -') && !hit.startsWith('Not Approved - Cannot Attach') ? 'Not Approved - Other Issues' : hit;
          break;
        } }
      }
      mix[s] = (mix[s] || 0) + 1;
    ...
    const glow = L.geoJSON(makeValid(feature), {
      pane: 'jobGlow',
      renderer: SVG_GLOW,
      style: { color: col, weight: 10, opacity: 0.20, fillColor: col, fillOpacity: 0.10 }
    }).addTo(map);

    const edge = L.geoJSON(makeValid(feature), {
      pane: 'jobEdge',
      renderer: SVG_EDGE,
