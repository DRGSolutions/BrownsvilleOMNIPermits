...
(function () {
  ...
  function signalWatchStart() {
    // Start watcher in THIS tab
    try { window.dispatchEvent(new CustomEvent("watch:start")); } catch { }
    ...
    // If this tab was opened by another tab (same-origin), notify opener too
    try {
      if (window.opener && typeof window.opener.dispatchEvent === "function") {
        window.opener.dispatchEvent(new CustomEvent("watch:start"));
      }
    } catch { }
  }
   ...
   * Returns: { notes, proposal } where proposal is the extracted number (or null).
   */
   ...
   * - If note already has a proposal number, replace the numeric token with proposalNum.
   * - If note has the number but no "proposal" word, normalize to "Proposal <num>".
   * - If note has no proposal number at all, append "Proposal <num>" on a new line.
   */
  ...
  // IMPORTANT: normalize the permit_notes field BEFORE watch.js reads the form (watch.js runs on capture)
  document.addEventListener("click", (ev) => {
    const t = ev.target;
    if (!t || t.id !== "btnSavePermit") return;

    const el = $("#permit_notes");
    if (!el) return;

    const raw = String(el.value ?? "");
    const norm = normalizeProposalInNotes(raw);
    if (norm.notes !== raw) el.value = norm.notes;
  }, true); // capture = true
  ...
  async function onSavePermit(e) {
    ...
    // Normalize notes (also returns proposal number, if present)
    ...
    const existing = permits.find(r => String(r?.permit_id) === String(f.permit_id));
    const exists = !!existing;

    // For updates, allow notes-only edits by NOT requiring pole keys.
    // If form is missing important fields, backfill from existing record so we don't accidentally blank required fields.
    if (exists && existing) {
      if (!f.job_name) f.job_name = existing.job_name;
      if (!f.tag) f.tag = existing.tag;
      if (!f.SCID) f.SCID = existing.SCID;

      if (!f.permit_status) f.permit_status = existing.permit_status;
      if (!f.submitted_by) f.submitted_by = existing.submitted_by;
      if (!f.submitted_at) f.submitted_at = existing.submitted_at;
    }

    // Only require pole keys when creating a NEW permit
    if (!exists && (!f.job_name || !f.tag || !f.SCID)) {
      msg('<span class="err">Missing pole keys.</span>');
      return;
    }

    // Require these for NEW permits (backend schema requires them)
    if (!exists) {
    ...
    // Build the primary change
    let primary;
    if (exists) {
      // For updates, keep the patch minimal and never send empty required fields.
      const patch = { notes: f.notes || "" };

      if (f.permit_status) patch.permit_status = f.permit_status;
      if (f.submitted_by) patch.submitted_by = f.submitted_by;
      if (f.submitted_at) patch.submitted_at = f.submitted_at;

      // Include pole keys only if we have them (never blank them)
      if (f.job_name) patch.job_name = f.job_name;
      if (f.tag) patch.tag = f.tag;
      if (f.SCID) patch.SCID = f.SCID;

      primary = { type: "update_permit", permit_id: f.permit_id, patch };
    } else {
      primary = {
        type: "upsert_permit",
        permit: {
    ...
    if (norm.proposal) {
      ...
      const all = permits.filter(r => basePermitId(r?.permit_id) === base);
  ...
  async function onDeletePermit(e) {
    ...
    try {
      ...
      const data = await callApi({
        ...
        reason: `Delete ${id}`,
        changes: [{ type: "delete_permit", permit_id: id }]
      });
  ...
  function updateAssignOnlyVisibility() {
    ...
    document.querySelectorAll(".assign-only")
  ...
  async function onMassApply(e) {
    ...
    // --- NOTES mass-update (normalize proposal format) ---
    const notesToApplyRaw = ($("#massNotes")?.value || "").trim();
    const massNorm = normalizeProposalInNotes(notesToApplyRaw);
    const notesToApply = massNorm.notes.trim();
    const wantNotes = !!notesToApply;
    ...
    if (wantNotes && mode !== "assign") {
      ...
      if (withNotes > 0) {
        ...
        // If Cancel: still update status, and only add notes to permits that currently have no notes.
      }
    }
    ...
    try {
      ...
      console.error(err);
      setMassMsg(`<span class="err">${err.message}</span>`);
    } finally {
  ...
  function onAdvancedMapClick() {
    const el = $("#fJob");
    const job = (el && el.value && el.value !== "All") ? el.value : "";
    if (!job) return;
  ...
  function wireButtons() {
    ...
    if (massApply) {
      ...
      massApply.removeEventListener("click", onMassApply);
      massApply.addEventListener("click", onMassApply);
    }

    const massMode = $("#massMode");
    if (massMode) {
      massMode.removeEventListener("change", updateAssignOnlyVisibility);
      massMode.addEventListener("change", updateAssignOnlyVisibility);
    }
